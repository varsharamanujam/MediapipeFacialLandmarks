...
def right_iris_position(iris_center, right_point, left_point):
    ...
    if ratio <= 0.45: # if ratio is less than 0.42
        IrisPosition = "Right" # Iris position is right
    elif ratio > 2.7 and ratio <= 3.2: # if ratio is between 2.7 and 3.2
        IrisPosition = "Center" # Iris position is center
    elif ratio : # if ratio is greater than 3.2
        IrisPosition = "Left" # Iris position is left
    return IrisPosition, ratio # return Iris position and ratio


def left_iris_position(iris_center, right_point, left_point): # same as right_iris_position
    ...
    IrisPosition = "" # Iris position
    if ratio <= 0.42: # if ratio is less than 0.42
        IrisPosition = "Left" # Iris position is left
    elif ratio > 2.7 and ratio <= 3.2: # if ratio is between 2.7 and 3.2
        IrisPosition = "Center" # Iris position is center   
    else: # if ratio is greater than 3.2
        IrisPosition = "Right" # Iris position is right
    return IrisPosition, ratio # return Iris position and ratio
...
with map_face_mesh.FaceMesh(
    ...
    while True: # looping until break
        ...
        if results.multi_face_landmarks: # if there are multiple faces
            ...
            iris_pos_r, ratio_r = right_iris_position(center_right, meshpoints[R_H_RIGHT], meshpoints[R_H_LEFT][0]) # getting right iris position and ratio
            iris_pos_l, ratio_l = left_iris_position(center_left, meshpoints[L_H_RIGHT], meshpoints[L_H_LEFT][0]) # getting left iris position and ratio
            
            cv2.putText(frame, "Iris Position Left: {} ;Ratio: {} Iris Position Right: {} ;Ratio: {}".format(iris_pos_l,ratio_l,iris_pos_r,ratio_r), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) # putting text on frame
